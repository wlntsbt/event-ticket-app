...
export const Header = () => {
  ...
  <div className="fixed z-50 border-b border-black bg-transparent backdrop-blur-sm w-full h-[50px] px-[25px] flex justify-between items-center lg:px-[100px]">
      <Link href="/">
        <div className="text-white tracking-widest drop-shadow-[2px_2px_2px_rgba(0,0,0,0.5)] text-lg hover:text-black font-bold" onClick={() => setToggleMenu(false)}>
          LESGOIN!
        </div>
      </Link>
      ...
      <div className="flex gap-5 items-center lg:hidden md:hidden">
        ...
        <div>
          {!toggleMenu && <RxHamburgerMenu className="cursor-pointer text-2xl" onClick={() => setToggleMenu(true)} />}
          {toggleMenu && <AiOutlineClose className="cursor-pointer text-2xl" onClick={() => setToggleMenu(false)} />}
          {toggleMenu && (
            <ul className="fixed bg-transparent backdrop-blur-lg backdrop-grayscale flex flex-col w-screen h-screen inset-0 mt-[50px] pt-5 text-center text-2xl leading-[100px]">
              <Link href={'/auth/login'}>
                <li onClick={() => setToggleMenu(false)} className="font-bold hover:text-white">LOGIN</li>
              </Link>
              <Link href={'/auth/register'}>
                <li onClick={() => setToggleMenu(false)} className="font-bold hover:text-teal-500">
                  REGISTER
                </li>
              </Link>
