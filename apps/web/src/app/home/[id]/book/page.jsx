...
export default function BookingPage() {
  ...
  const points = userPromo.userPromo?.Points.filter(
    (x) => !isPast(new Date(x.expiredAt)),
  );
  const promos = userPromo.eventPromo?.filter(
    (x) => !isPast(new Date(x.expiredAt)),
  );
  const vouchers = userPromo.userPromo?.Vouchers.filter(
    (x) => !isPast(new Date(x.expiredAt)),
  );
  ...
  const handlePromoSet = (id, amount) => {
    if (usePromos === 0 || usePromos !== id) {
      setUsePromos(id);
      setUseVouchers(0);
      setVoucherAmount(amount);
    } else {
  ...
  return (
    <div className="pt-[50px] flex-col gap-3 w-full">
      ...
      <div className="w-full lg:flex pt-8">
        {/* left section for desktop */}
        <div className="px-5 lg:flex-col lg:flex lg:w-1/2 lg:pl-[100px] lg:justify-start">
          <div>
            {/* event details */}
        ...
        <div className="pt-5 px-5 lg:w-1/2 lg:pr-[100px] lg:px-0">
          ...
          <div>
            <div className="py-2 flex justify-between items-center">
              ...
              <Switch
                disabled={true}
                checked={useVouchers || usePromos ? true : false}
              ></Switch>
            </div>
            ...
            {promos?.map((x, i) => (
              <div
                ref={voucherRef}
                onClick={() => handlePromoSet(x.id, Number(x.amount))}
                className={`${
                  x.id === usePromos
                    ? 'border-purple-500 border-2 rounded-lg'
                    : 'border-purple-300 border-dashed border rounded-lg'
